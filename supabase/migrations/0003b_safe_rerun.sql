-- Safe re-run of migration 0003: skips anything that already exists
...
create index if not exists status_history_asset_id_changed_at_idx
  on status_history (asset_id, changed_at desc);
...
create index if not exists activity_logs_created_idx
  on activity_logs (created_at desc) where deleted_at is null;
...
create index if not exists updates_asset_id_idx
  on updates (asset_id, created_at desc) where deleted_at is null;
...
create index if not exists tasks_asset_id_idx
  on tasks (asset_id) where deleted_at is null;
...
-- RLS + policies (wrapped to skip if already exists)
